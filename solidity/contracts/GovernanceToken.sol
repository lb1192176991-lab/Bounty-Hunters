...
contract GovernanceToken is ERC20, Ownable {
    ...
    // FIXED: Uses msg.sender instead of tx.origin to prevent phishing attacks
    ...
    // FIXED: Uses onlyOwner modifier from OpenZeppelin Ownable
    function snapshot() external onlyOwner {
        // snapshot logic placeholder
    }

    function getVotingPower(address account) public view returns (uint256) {
        // FIXED: Account for delegated votes properly
        uint256 selfBalance = balanceOf(account);
        uint256 delegatedFromOthers = delegatedPower[account];
        return selfBalance + delegatedFromOthers;
    }
